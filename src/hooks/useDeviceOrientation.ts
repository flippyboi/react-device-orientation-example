...
export const useDeviceOrientation = () => {
    const [or, setOr] = React.useState<{ gamma: number; beta: number; alpha: number }>({ gamma: 0, beta: 0, alpha: 0 });

    const handleOrientation = React.useCallback(({ alpha, beta, gamma }: DeviceOrientationEvent) => {
        const format = (n: number | null) => Number.parseFloat((n ?? 0).toFixed(2))
    ...
    React.useEffect(() => {
      ...
      React.useEffect(() => {
        if (isInHand) {
          if (swipeState === 'right') {
            window.dispatchEvent(new CustomEvent('gestureSwipe', { detail: {
              dir: 'right'
            } }));
